...
- **멀티 에이전트 dev-cycle** — 구현과 적대적 검수를 서로 다른 모델로 교차시키고, 배포는 사람이 게이트하는 팀 워크플로우 엔진
- **백엔드 아키텍처** — 느슨한 헥사고널(Ports & Adapters) · 도메인 주도 에러 설계 · 팀 코드 컨벤션 정립
- 사이드로 AI 에이전트 기반 서비스 실험 (소설 작성 에이전트 외)
